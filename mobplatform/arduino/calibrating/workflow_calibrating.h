 ...
 * //++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * 07.12.2023
 * Проверяем команду setPID, а то есть подозрения...
 * ...
 * setPID - проходит, все нормально.
 * - Имеем следующие подозрения :
 *   Перестала работать ( с чего вдруг ?!) ф-ция str.toCharArray() - поэтому не доходит строка "stop",
 *   которая есть индикатор завершения текущего цикла.
 *   ...
 *   Но ведь в предыдущих командах, до захода в ф-цию pid(), нормально работает str.toCharArray(), данные есть в файле.
 *
 * - Время Tfull установлено в 2000мс., pidTime = 300мс. При этом наблюаем заход в ф-цию pid() аж 14 раз, а должно быть 2000/300, т.е. 6-7-8 раз.
 *   Откуда столько заходов в pid() ???
 *
 * - в Поле "status" по-прежнему ересь
 * - в Поле "debuginfo", однако, наблюдаем символы "pid", что радует, однако эти символы где-то в середине строки, а должны быть в начале.
 * ...
 * data.csv - переписывается, данные с первого захода "start"  - есть.
 * ...
 * ...
 * НАШЁЛ !!!
 * Как и предполагалось, случился рассинхрон шагов приём/передача.
 * В обработчике parse_command() для команды (message == "start") была добавлена отладочная печать, видимо с целью "быстренько глянуть",
 * Serial.println(str), а штатная отправка  Serial.write((byte*)&data, sizeof(data)); - не была закомментирована.
 * И обработчик команды на стороне ПК, конечно, не смог распарсить таку строку...
 * Ошибка поиска в том, что не там начал искать.
 * НАчал искать в ф-ции pid(), а надо было сразу начинать с той точки, где происходит слом стройности/целостности данных в data.csv - т.е.
 * с команды "start"
 * ...
 * - Убрал лишние volatile объявления.
 *
 * Сохраняем на git
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * 06.12.2023
 * После незначительных изменений в коде python и, соответственно, в коде arduino. Система ПО+Робот перестала работать как надо.
 * После выполнения калибровки выполняется(!) 1 раз команда start и на этом зависает. ПК ждет каких-то данных в serial port, а робот их не дает.
 * По в ожидании.
 *
 * Убрал все warnings в arduino IDE (!)
 *
 * Запись символов в структуру где-то сбоит, ПО на стороне ПК не ловит ключевое слово 'stop' и поэтому все зависает.
 * data.mytext == b'\x07>J\x82\xc4pid\x00ted\x00`q\x00\x00\x00\x98\xf8\xc5\x00\x00\xa0B\xb2c\x10\xc5Every move you'
 *
 * не меняется значение millis() !!!!
 * 
 * //++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * 24.11.2023
 * Kp = 0.0185, Ki = 0.008, Kd = 0.0175 tPID=300ms
 * Вот с этими параметрами на вису работает хорошо.
 * Init values for speed are  26 ,  25
 *
 * Попробуем использовать deltaT в мс, а не в секундах как сейчас. Тогда и PID коэффициннты будут больше по модулю.
 * И возможно, точнее управление.
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * 21.11.2023
